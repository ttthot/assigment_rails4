...
class FormSubmission {
  ...
  async start() {
    ...
    if (typeof confirmationMessage === 'string') {
      const confirmMethod =
        typeof config.forms.confirm === 'function' ? config.forms.confirm : FormSubmission.confirmMethod;
...
var Idiomorph = (function () {
  ...
  function morph(oldNode, newContent, config = {}) {
    ...
    const morphedNodes = saveAndRestoreFocus(ctx, () =>
      withHeadBlocking(ctx, oldNode, newNode, (ctx) => {
...
const linkToTheSamePage = (link) =>
  link.pathname + link.search === document.location.pathname + document.location.search || link.href.startsWith('#');
...
function canRefreshFrame(frame) {
  return (
    frame instanceof FrameElement && frame.src && frame.refresh === 'morph' && !frame.closest('[data-turbo-permanent]')
  );
}
...
function encodeMethodIntoRequestBody(event) {
  if (event.target instanceof HTMLFormElement) {
    const {
      target: form,
      detail: { fetchOptions: fetchOptions },
    } = event;
    form.addEventListener(
      'turbo:submit-start',
      ({
        detail: {
          formSubmission: { submitter: submitter },
        },
      }) => {
